...
#using cross-validated option to ascertain lambda
calc.gene.weights.cv = function(gene="",values.list,target,index=NULL,target.index=NULL) 
{
 if(is.null(index)) #if index isn't specified, determine the index by the gene name
 {
  index = which(rownames(values.list[[1]]) == gene)
 ...
 lambda.range = seq(0.5,4.5,by=0.2)
 
 if(d2>0)
 {

  D = cbind(Y,X.mat)
  test=createDataPartition(D[,1],times=10,p=0.8,list=FALSE)
  avg.cv.r = vector(length=length(lambda.range))
  for(j in 1:length(lambda.range))
  {
   cv.r = vector(length=dim(test)[2])
   for(i in 1:dim(test)[2])
   {
    train.ids = test[,i]
    D.train= D[train.ids,]
    iden = diag(lambda.range[j],d2)
    C = apply(D.train,2,inv.norm)
    C.cov = cor(C)
    C.x = C.cov[-1,-1]+iden
    C.xy = C.cov[1,-1]
    H = ginv(C.x)

    cor.xy = as.numeric(cor(Y[train.ids],X.mat[train.ids,]))
    if(!is.null(cv.index))
    {
     cor.xy[cv.index] = sqrt(cv.values[cv.index])
    }
    ...
    D.test = D[-train.ids,]
    Y.test = D.test[,1]
    X.test = as.matrix(D.test[,-1])
    Y.test.pred = X.test%*%as.numeric(wts)
    cv.r[i] = cor(Y.test.pred,Y.test)^2
   }
   avg.cv.r[j] = mean(cv.r)
  }
  k = which.max(avg.cv.r)
  #calculate the weights based on the best cross-validated lambda
  #C = apply(D,2,center.values)
  iden = diag(lambda.range[k],d2)
...
for(i in 1:dim(tissue.values[[1]])[1])
{
 ...
 #}
 target.weights[i,] = calc.gene.weights.cv(values.list=tissue.values,target=target.final,index=i,target.index=target.index)
}
