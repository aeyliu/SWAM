...
print( paste("Reading in prediXcan output...") )
...
calc.pairwise.cor = function(predicted,measured)
{
 gene = rownames(predicted)
 cor.pw = vector(length=dim(measured)[1])
 for(i in 1:dim(measured)[1])
 {
  cor.pw[i] = cor(predicted[i,],measured[i,],method="spearman")
 }
 df.out = data.frame(gene,cor.pw)
 return(df.out)
}

calculate.test.expr = function(wts,test.list)
{
 expr.cumulative = 0
 for(i in 1:length(test.list))
 {
  expr.cumulative = test.list[[1]] * wts[,i] + expr.cumulative
 }
 return(expr.cumulative)
}


#generate cross-validated set of "tissue.values"
set.seed(123)
n.ids = dim(tissue.values[[1]])[2]
test=createDataPartition(1:n.ids,times=10,p=0.8,list=FALSE)

lambda.range = seq(1,10,by=1)
cv.accuracy = matrix(nrow = length(lambda.range),ncol = 10)
cv.accuracy.mean = matrix(nrow = length(lambda.range),ncol = 10)

for(i in 1:dim(test)[2])
{
 print(paste("Testing cv set: ", i, " of 10",sep=""))
 tissue.values.train = vector(mode="list",length=length(tissue.values))
 tissue.values.test = vector(mode="list",length=length(tissue.values))

 for(j in 1:length(tissue.values))
 {
  tissue.values.train[[j]]=tissue.values[[j]][,test[,i]]
  tissue.values.test[[j]] = tissue.values[[j]][,-test[,i]]
 }
 target.final.train = target.final[,test[,i]]
 target.final.test = target.final[,-test[,i]]

 for(j in 1:length(lambda.range))
 {
  target.weights = matrix(nrow = dim(tissue.values[[1]])[1], ncol = length(tissue.values))
  for(k in 1:dim(tissue.values[[1]])[1])
  {
   target.weights[k,] = calc.gene.weights2(values.list=tissue.values.train,target=target.final.train,index=k,target.index=target.index,diag.val = lambda.range[j])
  }
  #now use these target.weights to meta-predict expression levels for the training set
  test.expr = calculate.test.expr(target.weights,tissue.values.test)
  #calculate pairwise correlation between measured/predicted
  pw.cor = calc.pairwise.cor(test.expr,target.final.test)
  cv.accuracy[j,i] = length(which(pw.cor[,2]>0.1)) #roughly the number of significantly imputable genes

 }

}

apply(cv.accuracy,1,mean)
...
for(i in 1:dim(tissue.values[[1]])[1])
{
 ...
 #}
 target.weights[i,] = calc.gene.weights2(values.list=tissue.values,target=target.final,index=i,target.index=target.index)
}
